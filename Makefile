...
go_ldflags := -s -w
ifneq ($(GIT_TAG),)
go_ldflags += -X github.com/networkservicemesh/cmd-csi-driver/internal/version.gitTag=$(GIT_TAG)
endif
ifneq ($(GIT_COMMIT),)
go_ldflags += -X github.com/networkservicemesh/cmd-csi-driver/internal/version.gitCommit=$(GIT_COMMIT)
endif
ifneq ($(GIT_DIRTY),)
go_ldflags += -X github.com/networkservicemesh/cmd-csi-driver/internal/version.gitDirty=$(GIT_DIRTY)
endif
...
docker-build:
	docker build \
		...
		--target cmd-csi-driver \
		-t ghcr.io/networkservicemesh/cmd-csi-driver:devel \
		.

.PHONY: build
build: | bin
	CGO_ENABLED=0 go build -ldflags ${go_ldflags} -o bin/nsm-csi-driver ./cmd/nsm-csi-driver
